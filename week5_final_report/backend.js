// =================================================================
// Backend Server "Eventify" - Week 4 Integration (FINAL REVISED)
// =================================================================
...
// =================================================================
// 4. EVENTS (CORE CRUD + SEARCH + STATUS REGISTRASI)

// GET EVENTS (Search & Check Registration Status)
app.get('/api/events', async (req, res) => {
    try {
        const { search, userId } = req.query; 

        // Query dimodifikasi untuk mengecek status pendaftaran user (is_registered) DAN status ulasan (has_reviewed)
        let query = `
            SELECT e.*, c.nama_kategori, o.nama_organizer,
            (SELECT COUNT(*) FROM Registrations r WHERE r.event_id = e.event_id AND r.user_id = ?) AS is_registered,
            (SELECT COUNT(*) FROM Feedback f WHERE f.event_id = e.event_id AND f.user_id = ?) AS has_reviewed
            FROM Events e
        ...
        const uid = userId || 0;
        const params = [uid, uid]; // Parameter ganda untuk dua tanda tanya (?) di atas
